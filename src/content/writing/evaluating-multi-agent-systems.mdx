...
<Callout type="tldr">
Three eval modes &mdash; output, trajectory, side-effect. Score every run on all three.
Look at the joint distribution. If you only grade the final answer, you'll ship agents
that pass eval and break in production.
</Callout>
...
<Callout type="warning" title="Do not skip eval">
The teams that ship agentic systems and the teams that ship demos diverge here, and the
gap compounds. Every model upgrade, every prompt change, every new tool integration breaks
something. Without a measurement system you can run on every change, you&rsquo;re not
engineering &mdash; you&rsquo;re vibe-checking.
</Callout>
